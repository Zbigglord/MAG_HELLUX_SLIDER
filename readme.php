...
<?php $slider = Hellux_Slider_Model_Slider::getSliderData(1); //1 is slider id so we may have as meny we want ?>
    <div id="slider">
		...
		<!-- maybe we want indicators (if not we may want just comment it out)-->
		<ol class="carousel-indicators">
		  <?php $i = 0;?>
          <?php foreach($slider as $item): ?>
		    <li class="<?php if($i == 0):?>active<?php endif;?>" data-slide-to="<?=$i?>" data-target="#helluxSlider"><?= $i+1; ?></li><!-- comments remove spaces between inline-block elements -->
		   <?php $i++;?>
           <?php endforeach; ?>
		</ol>		
            ...
            <div class="carousel-inner" role="listbox">
                ...
                <img class="img-reponsive carousel-image" src="<?= $item['item_address'].''.$item['item_file_name'] ?>" />
				<div class="slider-caption-container">
				 ...
				 <p class="slider-button-p">
                    <a class="btn btn-lg btn-primary slider-button" role="button" href="http://<?= $item['item_link'] ?>" target="_blank"><?= $item['button_text'] ?></a>
                 </p>
